...
class FunctionCall(ASTNode):
    ...
    def evaluate(self, scope):
        ...
        for name, expr in zip(function.args, self.args):
            call_scope[name] = expr.evaluate(new_scope)
        result = None
        for stmt in function.body:
            result = stmt.evaluate(call_scope)
        return result
...
class BinaryOperation(ASTNode):
    ...
    def __init__(self, lhs, op, rhs):
        if op not in BinaryOperation.OPERATORS:
            raise NotImplementedError('Unknown operator: ' + str(op))
        self.lhs = lhs
        self.op = op
        self.rhs = rhs

    def evaluate(self, scope):
        result = BinaryOperation.OPERATORS[self.op](
...
class UnaryOperation(ASTNode):
    ...
    def __init__(self, op, expr):
        if op not in UnaryOperation.OPERATORS:
            raise NotImplementedError('Unknown operator: ' + str(op))
        self.op = op
        self.expr = expr

    def evaluate(self, scope):
        result = UnaryOperation.OPERATORS[self.op](
